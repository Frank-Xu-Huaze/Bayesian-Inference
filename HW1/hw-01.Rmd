...
***General instructions for homeworks***: Upload an R Markdown file, named with your net ID, to Sakai. You will give the commands to answer each question in its own code block, which will also produce plots that will be automatically embedded in the output file. Each answer must be supported by written statements as well as any code used. Your code must be completely reproducible and must compile. (If you have additional files to upload, then you must **zip** them all as **one file** named with your net ID to upload them.)
